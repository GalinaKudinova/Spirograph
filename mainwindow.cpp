...
void MainWindow::drawAxes()
{
    ...
    scene->addRect(-half_X,-half_Y,MAX_X,MAX_Y, QPen(Qt::DotLine));
}
...
QPoint MainWindow::drawCircle(QPoint center, qreal radius, qreal angle, QColor color)
{
    ...
    for(i=0;i<static_cast<int>(angle); i++){
        ...
        //QPoint end(static_cast<int>(x),static_cast<int>(y));
...
void MainWindow::on_doubleSpinBox_valueChanged(double arg1)
{
    ...
    ui->label->setText(qsTemp.arg(arg1,0,'g',3));
    QPoint O(0,0);
    int radius=ui->spinBox->value();
    scene->clear();
    drawAxes();
    QPoint newStart=drawCircle(O,radius, arg1,Qt::magenta);
    qDebug()<<drawCircle(newStart,radius/2,arg1, Qt::cyan);
    //drawCircle(O,radius,arg1);
}
